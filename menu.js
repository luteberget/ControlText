 ...
 * 
**/  
function MkMenu(container, config) {
    config = config || {};
    ...
    var createDropDownController = function(elem) {
        ...
        var p = {
            ...
            refresh : function(token, array, providers) {
                ...
                // Create custom item providers rows
                for(var i = 0; i < providers.length; i++) {
                  var provider = providers[i];
                  if(!(provider.items.length > 0)) continue;
                  var headerRowText = provider.renderHeader();
                  if(String(headerRowText.length) > 0) {
                    var headerDiv = document.createElement('div');
                    headerDiv.innerHTML = headerRowText;
                  ...
                  for(var j = 0; j < provider.items.length; j++) {
                    ...
                    div.innerHTML = String(provider.render(j));
                    div.className = "provider-item provider-item-" + provider.name;
                    div.__idx = j;
                    div.__provider = provider;
                    div.onmousedown = function() { this.__provider.execute(this.__idx); }
                    rows.push(div); 
                    anyCustom = true;
                ...
                if (!anyCustom && rows.length===1 && token === rows[0].__hint) {
                    return; // do not show the dropDown if it has only one element which matches what we have just displayed.
                }
                
                if (!anyCustom && rows.length<2) return; 
                p.highlight(0);
    ...
    var rs = { 
        ...
        repaint : function() {
            ...
            dropDownController.refresh(token, rs.options, rs.otherOptions);
        }
    };
    
    var registerOnTextChangeOldValue;
    var cursorPositionOldValue;
    ...
    **/
    var registerOnTextChange = function(txt, callbackChange, callbackCursor) {
        registerOnTextChangeOldValue = txt.value;
        var handler = function() {
            var value = txt.value;
            var cursor = txt.selectionStart;
            console.log("got " + String(value) + " -- at " + String(cursor));
            if (registerOnTextChangeOldValue !== value) {
                console.log("then called Chang");
                registerOnTextChangeOldValue = value;
                callbackChange(value);
                cursorPositionOldValue = cursor;
            } else if(cursorPositionOldValue != cursor) {
                console.log("then called CursorChanged");
                callbackCursor();
                cursorPositionOldValue = cursor;
            }
        };
        ...
        if (txt.addEventListener) {
            ...
            txt.attachEvent('onclick',handler); // IE<9
            txt.attachEvent('onfocus',handler); // IE<9
        }
    };
